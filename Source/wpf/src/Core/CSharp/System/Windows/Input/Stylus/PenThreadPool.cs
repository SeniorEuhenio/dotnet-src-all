        ...
        private PenThread GetPenThreadForPenContextHelper(PenContext penContext)
        {
            ...
            while (ignoredThreads.Count < MAX_PENTHREAD_RETRIES)
            {
                ...
                else
                {
                    ...
                    EventTrace.EasyTraceEvent(EventTrace.Keyword.KeywordInput,
                        EventTrace.Event.PenThreadPoolThreadAcquisition,
                        "Retrying PenThread selection due to re-entrancy");
                }
            }
            ...
            if (selectedPenThread == null)
            {
                EventTrace.EasyTraceEvent(EventTrace.Keyword.KeywordInput,
                            EventTrace.Event.PenThreadPoolThreadAcquisition,
                            "Retry limit reached when acquiring PenThread");
